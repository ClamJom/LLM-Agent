...
class ImagePromptInitializer:
    ...
    def add(self, image_url: str, is_url = False):
        if is_url:
            ...
            self.images.append({
                "type": "image_url",
                "image_url": {"url": f"data:image/jpeg;base64,{self.__to_base_64(image_url, is_url)}"}
            })
    ...
    def attachment_handler(self):
        ...
        for attachment in self.attachments:
            if str(attachment["type"]).find("image") != -1:
                ...
                content = ""
                with open(os.path.join(UPLOAD_PATH, attachment["name"]), 'r', encoding="utf-8") as f:
                    content = f.read()
                    f.close()
...
def tools_result(toolRequest: ToolRequest):
    try:
        ...
        return json.dumps([{"name": "error", "result": "工具调用出错，请检查参数与格式"}])
...
def start_new_conversation(title: str = Body(..., title="title", embed=True, description="标题")):
    return do.insert_conversation(title, datetime.now())

@app.post("/save/conversation")
def save_conversation(conversation_id: str = Body(..., title="conversation_id", embed=True, description="会话ID"),
                    messages: list[dict] = Body(..., title="messages", embed=True, description="消息列表")):
...
def update_conversation(conversation_id: str = Body(..., title="conversation_id", embed=True, description="会话ID"),
                        messages: list[dict] = Body(..., title="messages", embed=True, description="消息列表")):
    do.update_messages(conversation_id, messages)
...
def get_title(messages: list[dict]):
    title_summerizer_settings = {
        "stream": True,
        "system_prompt": prompt.TITLE_PROMPT
    }
    model = OpenAi.OpenAi(API_URL, API_KEY,
                            TITLE_SUMMERIZER,
                            **title_summerizer_settings)
